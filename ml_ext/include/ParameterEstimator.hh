...
public:
    ParameterEstimator(const std::string& config_path)
    {
        ...
        const std::string& model_path = base_directory_ + config.at("model_file").get<std::string>();
    ...
    ParameterEstimator(const std::string& base_directory, const json& config)
    {
        ...
        feature_extractor_ = std::make_unique<TelFeatureExtractor>(config.at("features"));
        const std::string& model_path = base_directory + config.at("model_file").get<std::string>();
        predictor_ = std::make_unique<LGBMPredictor>(model_path);
    }
