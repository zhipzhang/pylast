...
static bool registered_energy_regressor = []() {
    ReconstructorFactory::instance().register_reconstructor("EnergyRegressor", [](const SubarrayDescription& subarray, const json& config) -> std::unique_ptr<Reconstructor> {
...
void EnergyRegressor::operator()(ArrayEvent& event)
{
    ...
    {
        spdlog::error("HillasReconsturctor is not avaliable, it's better to have it as initial value for EnergyRegressor");
        throw std::runtime_error("HillasReconsturctor is not avaliable, it's better to have it as initial value for EnergyRegressor");
    }
    ...
    for(int i = 0; i < telescopes.size(); i++)
    {
        ...
        double energy= energy_regressor.predict(rec_offset, const_cast<const ArrayEvent&>(event), tel_id);
    ...
    energy_reco.estimate_energy = (tel_energies.array() * weights.array()).sum() / weights.sum();
