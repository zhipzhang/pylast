...
TEST_CASE("TEST_TAILCUTS_CLEAN") {
  ...
  CameraGeometry camera("test", num_pixels, pix_x.data(), pix_y.data(),
                        pix_area.data(), pix_type.data(), 0);
  // Test center pixel (5) should have 4 neighbors
  SUBCASE("EMPTY_IMAGE") {
    auto empty_image = Eigen::VectorXd::Zero(num_pixels);
    auto clean_image =
        TailcutsCleaner::tailcuts_clean(camera, empty_image, 1, 1);
  ...
  SUBCASE("CONSTANT_IMAGE") {
    ...
    auto clean_image =
        TailcutsCleaner::tailcuts_clean(camera, constant_image, 1, 1);
  ...
  SUBCASE("SOLO_ABOVE_THRESHOLD") {
    ...
    solo_image(10) = 10;
    auto clean_image =
        TailcutsCleaner::tailcuts_clean(camera, solo_image, 8, 1);
  ...
  SUBCASE("KEEP_ISOLATED_PIXELS") {
    Eigen::VectorXd keep_isolated_image =
        Eigen::VectorXd::Constant(num_pixels, 1);
    keep_isolated_image(10) = 10;
    keep_isolated_image(6) = 5;
    keep_isolated_image(9) = 5;
    keep_isolated_image(0) = 10;
    auto clean_image = TailcutsCleaner::tailcuts_clean(
        camera, keep_isolated_image, 8, 2, true);
  ...
  SUBCASE("NO_KEEP_ISOLATED_PIXELS") {
    Eigen::VectorXd no_keep_isolated_image =
        Eigen::VectorXd::Constant(num_pixels, 1);
    ...
    auto clean_image = TailcutsCleaner::tailcuts_clean(
        camera, no_keep_isolated_image, 8, 2, false, 2);
...
TEST_CASE("TEST_LEAKAGE_PARAMETERS") {
  ...
  SUBCASE("EMPTY_IMAGE") {
    ...
    auto leakage_parameters =
        ImageProcessor::leakage_parameter(camera, empty_image);
  ...
  SUBCASE("CONSTANT_IMAGE") {
    auto constant_image = Eigen::VectorXd::Constant(num_pixels, 10);
    auto leakage_parameters =
        ImageProcessor::leakage_parameter(camera, constant_image);
...
TEST_CASE("TEST_MORPHOLOGY_PARAMETERS") {
  ...
  SUBCASE("EMPTY_MASk") {
    ...
    auto morphology_parameters =
        ImageProcessor::morphology_parameter(camera, empty_mask);
  ...
  SUBCASE("FULL_MASK") {
    ...
    auto morphology_parameters =
        ImageProcessor::morphology_parameter(camera, full_mask);
  ...
  SUBCASE("NORMAL_MASK") {
    Eigen::Vector<bool, -1> normal_mask =
        Eigen::Vector<bool, -1>::Zero(num_pixels);
    normal_mask(0) = true;
    normal_mask(1) = true;
    normal_mask(2) = true;
    ...
    normal_mask(24) = true;
    auto morphology_parameters =
        ImageProcessor::morphology_parameter(camera, normal_mask);
...
TEST_CASE("TEST_HILLAS_PARAMETER") {
  ...
  SUBCASE("ONLY_DIAGONAL_PIXELS") {
    ...
    diagonal_image(10) = 1;
    diagonal_image(15) = 1;
    auto hillas_parameters =
        ImageProcessor::hillas_parameter(camera, diagonal_image);
...
TEST_CASE("TEST_CUT_RADIUS") {
  ...
  CameraGeometry camera("test", num_pixels, pix_x.data(), pix_y.data(),
                        pix_area.data(), pix_type.data(), 0);
  auto pixel_mask =
      ImageProcessor::cut_pixel_distance(camera, 1, 2 * 180 / M_PI);
  CHECK(pixel_mask.array().cast<int>().sum() == 6);
  auto pixel_mask2 =
      ImageProcessor::cut_pixel_distance(camera, 1, 1 * 180 / M_PI);
