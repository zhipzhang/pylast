...
from pylast.histogram.base_histogram import BaseHistogramProfile, BaseHistogram1D
...
def initialize_histogram():
    energy_range = np.logspace(-1, 3, 21) 
    impact_parameter_range = np.linspace(0, 600, 31)
    offset_angle_range = np.linspace(0, 5, 6)
    ...
    angular_resolution_1 = BaseHistogram1D("angular_resolution_2tel", offset_angle_range, energy_range, np.linspace(0, 1, 1000))
    angular_resolution_2 = BaseHistogram1D("angular_resolution_3tel", offset_angle_range, energy_range, np.linspace(0, 1, 1000))
    angular_resolution_3 = BaseHistogram1D("angular_resolution_4tel", offset_angle_range, energy_range, np.linspace(0, 1, 1000))
    angular_resolution_4 = BaseHistogram1D("angular_resolution_5tel", offset_angle_range, energy_range, np.linspace(0, 1, 1000))
    return {
        ...
        "angular_resolution_1": angular_resolution_1,
        "angular_resolution_2": angular_resolution_2,
        "angular_resolution_3": angular_resolution_3,
        "angular_resolution_4": angular_resolution_4
    }
    ...
    for input_file in args.input:
        ...
        if (type(source) == SimtelEventSource):
            for event in source:
                ...
                for tel_id, _ in event.dl2.tels.items():
                    dl1 = event.dl1.tels[tel_id]
                ...
                if(event.dl2.geometry["HillasReconstructor"].is_valid):
                    if(len(event.dl2.geometry["HillasReconstructor"].telescopes) == 2):
                        histograms["angular_resolution_1"].fill(offset_angle, event.simulation.shower.energy,  np.rad2deg(event.dl2.geometry["HillasReconstructor"].direction_error ))
                    elif(len(event.dl2.geometry["HillasReconstructor"].telescopes) == 3):
                        histograms["angular_resolution_2"].fill(offset_angle, event.simulation.shower.energy, np.rad2deg(event.dl2.geometry["HillasReconstructor"].direction_error ))
                    elif(len(event.dl2.geometry["HillasReconstructor"].telescopes) == 4):
                        histograms["angular_resolution_3"].fill(offset_angle, event.simulation.shower.energy, np.rad2deg(event.dl2.geometry["HillasReconstructor"].direction_error ))
                    elif(len(event.dl2.geometry["HillasReconstructor"].telescopes) == 5):
                        histograms["angular_resolution_4"].fill(offset_angle, event.simulation.shower.energy, np.rad2deg(event.dl2.geometry["HillasReconstructor"].direction_error ))
