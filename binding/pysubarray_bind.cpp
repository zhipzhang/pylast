...
void bind_subarray_description(nb::module_ &m) {
  nb::class_<SubarrayDescription>(m, "SubarrayDescription")
      ...
      .def_prop_ro("get_pix_x",
                   [](const SubarrayDescription &self) -> Eigen::VectorXd {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     auto pix_x = self.tels.at(id_vecs[0])
                                      .camera_description.camera_geometry.pix_x;
                     double focal_length =
                         self.tels.at(id_vecs[0])
                             .optics_description.effective_focal_length;
                     return pix_x.array() / focal_length * 180 / M_PI;
                   })
      .def_prop_ro("get_pix_y",
                   [](const SubarrayDescription &self) -> Eigen::VectorXd {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     auto pix_y = self.tels.at(id_vecs[0])
                                      .camera_description.camera_geometry.pix_y;
                     double focal_length =
                         self.tels.at(id_vecs[0])
                             .optics_description.effective_focal_length;
                     return pix_y.array() / focal_length * 180 / M_PI;
                   })
      .def_prop_ro("get_pix_x_equivalent",
                   [](const SubarrayDescription &self) -> Eigen::VectorXd {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     auto pix_x = self.tels.at(id_vecs[0])
                                      .camera_description.camera_geometry.pix_x;
                     double focal_length =
                         self.tels.at(id_vecs[0])
                             .optics_description.equivalent_focal_length;
                     return pix_x.array() / focal_length * 180 / M_PI;
                   })
      .def_prop_ro("get_pix_y_equivalent",
                   [](const SubarrayDescription &self) -> Eigen::VectorXd {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     auto pix_y = self.tels.at(id_vecs[0])
                                      .camera_description.camera_geometry.pix_y;
      ...
      .def_prop_ro("get_pix_size",
                   [](const SubarrayDescription &self) -> double {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     double focal_length =
                         self.tels.at(id_vecs[0])
                             .optics_description.effective_focal_length;
                     auto pix_area =
                         self.tels.at(id_vecs[0])
                             .camera_description.camera_geometry.pix_area;
                     return sqrt(pix_area[0]) / focal_length * 180 / M_PI;
                   })
      .def_prop_ro("get_pix_size_equivalent",
                   [](const SubarrayDescription &self) -> double {
                     auto id_vecs = self.get_ordered_telescope_ids();
                     double focal_length =
                         self.tels.at(id_vecs[0])
  ...
  nb::class_<OpticsDescription>(m, "OpticsDescription")
      ...
      .def_ro("equivalent_focal_length",
              &OpticsDescription::equivalent_focal_length)
      .def_ro("effective_focal_length",
              &OpticsDescription::effective_focal_length)
  ...
  nb::class_<CameraReadout>(m, "CameraReadout")
      ...
      .def_ro("reference_pulse_sample_width",
              &CameraReadout::reference_pulse_sample_width)
  ...
  nb::class_<LACT1CameraGeometry, CameraGeometry>(m, "LACT1CameraGeometry")
      .def(nb::init<const std::string &>())
      .def_ro("camera_file_name", &LACT1CameraGeometry::camera_file_name)
      .def("convert_map_to_board_channel",
           &LACT1CameraGeometry::convert_map_to_board_channel)
      .def("convert_board_channel_to_map",
           &LACT1CameraGeometry::convert_board_channel_to_map);
}

NB_MODULE(_pylast_subarray, m) { bind_subarray_description(m); }
