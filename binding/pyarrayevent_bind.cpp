...
void bind_dl2_event(nb::module_ &m) {
  ...
  nb::class_<ReconstructedEnergy>(m, "ReconstructedEnergy")
      ...
      .def("__repr__", [](ReconstructedEnergy &self) {
        return fmt::format(
            "ReconstructedEnergy:\n  estimate_energy: {}\n  energy_valid: {}",
            self.estimate_energy, self.energy_valid);
      });
  ...
  nb::class_<ReconstructedGeometry>(m, "ReconstructedGeometry")
      ...
      .def_rw("tilted_core_uncertainty_x",
              &ReconstructedGeometry::tilted_core_uncertainty_x)
      .def_rw("tilted_core_uncertainty_y",
              &ReconstructedGeometry::tilted_core_uncertainty_y)
      ...
      .def("set_telescopes",
           [](ReconstructedGeometry &self, nb::list telescopes) {
      ...
      .def("__repr__", [](ReconstructedGeometry &self) {
        return fmt::format("ReconstructedGeometry:\n"
                           ...
                           self.core_pos_error, self.tilted_core_x,
                           self.tilted_core_y);
      });
  nb::class_<TelReconstructedParameter>(m, "TelReconstructedParameter")
      .def_ro("impact_parameters",
              &TelReconstructedParameter::impact_parameters)
      ...
      .def_prop_ro("impact",
                   [](TelReconstructedParameter &self) -> nb::object {
...
void bind_dl1_event(nb::module_ &m) {
  ...
  nb::class_<ImageParameters>(m, "image_parameters")
      ...
      .def("__repr__", [](ImageParameters &self) {
        return "ImageParameters: contains hillas, leakage, concentration, and "
  ...
  nb::class_<HillasParameter>(m, "hillas")
      ...
      .def("__repr__", [](HillasParameter &self) {
        return fmt::format("HillasParameter:\n  x: {}\n  y: {}\n  width: {}\n  "
                           "length: {}\n  phi: {}\n  intensity: {}",
                           self.x, self.y, self.width, self.length, self.phi,
                           self.intensity);
      });
  nb::class_<LeakageParameter>(m, "leakage")
      ...
      .def("__repr__", [](LeakageParameter &self) {
        return fmt::format(
            "LeakageParameter:\n  pixels_width_1: {}\n  pixels_width_2: {}\n  "
            "intensity_width_1: {}\n  intensity_width_2: {}",
            self.pixels_width_1, self.pixels_width_2, self.intensity_width_1,
            self.intensity_width_2);
      });
  nb::class_<ConcentrationParameter>(m, "concentration")
      ...
      .def_ro("concentration_pixel",
              &ConcentrationParameter::concentration_pixel)
      .def("__repr__", [](ConcentrationParameter &self) {
        return fmt::format("ConcentrationParameter:\n  concentration_cog: {}\n "
                           " concentration_core: {}\n  concentration_pixel: {}",
                           self.concentration_cog, self.concentration_core,
                           self.concentration_pixel);
      });
  ...
  nb::class_<IntensityParameter>(m, "intensity")
      ...
      .def("__repr__", [](IntensityParameter &self) {
        return fmt::format("IntensityParameter:\n  intensity_max: {}\n  "
                           "intensity_mean: {}\n  intensity_std: {}\n  "
                           "intensity_skewness: {}\n  intensity_kurtosis: {}",
                           self.intensity_max, self.intensity_mean,
                           self.intensity_std, self.intensity_skewness,
                           self.intensity_kurtosis);
      });
}
void bind_dl0_event(nb::module_ &m) {
  ...
  nb::class_<DL0Camera>(m, "DL0Camera")
      ...
      .def("__repr__", [](DL0Camera &self) {
        return fmt::format(
            "DL0Camera:\n  image shape: {}x{}\n  peak_time shape: {}x{}",
            self.image.rows(), self.image.cols(), self.peak_time.rows(),
            self.peak_time.cols());
      });
}
  ...
  nb::class_<R0Camera>(m, "R0Camera")
      ...
      .def("__repr__", [](R0Camera &self) {
        return fmt::format("R0Camera:\n  waveform shape: 2x{}x{}\n  "
                           ...
                           (self.waveform_sum)[0].rows(),
                           (self.waveform_sum)[0].cols());
      });
}
...
void bind_pointing_event(nb::module_ &m) {
  nb::class_<Pointing>(m, "Pointing")
      ...
      .def("__repr__", [](Pointing &self) {
        return fmt::format(
            "Pointing:\n  array_azimuth: {}\n  array_altitude: {}",
            self.array_azimuth, self.array_altitude);
      });
}
...
void bind_simulated_shower_array(nb::module_ &m) {
  nb::class_<SimulatedShowerArray>(m, "SimulatedShowerArray")
      ...
      .def_prop_ro("starting_grammage",
                   &SimulatedShowerArray::starting_grammage)
      .def_prop_ro("shower_primary_id",
                   &SimulatedShowerArray::shower_primary_id)
...
void bind_calib_event(nb::module_ &m) {
  nb::class_<CalibEvent>(m, "CalibEvent")
      .def_prop_ro("tels", &CalibEvent::get_tels)
      .def("__repr__", [](CalibEvent &self) {
        std::string repr = "CalibEvent:\n";
        auto tels = self.get_tels();
        return repr;
      });
  nb::class_<CalibCamera>(m, "CalibCamera")
      .def_ro("base", &CalibCamera::base)
      .def_ro("peak", &CalibCamera::peak)
      .def_ro("area", &CalibCamera::area)
      .def_ro("count_to_pe", &CalibCamera::count_to_pe)
      .def_ro("pe", &CalibCamera::pe)
      .def("__repr__", [](CalibCamera &self) {
        return fmt::format(
            "CalibCamera:\n  base shape: {}x{}\n  peak shape: "
            "{}x{}\n  area shape: {}x{}\n  count_to_pe shape: {}x{}",
            self.base.rows(), self.base.cols(), self.peak.rows(),
            self.peak.cols(), self.area.rows(), self.area.cols(),
            self.count_to_pe.rows(), self.count_to_pe.cols());
      });
}
