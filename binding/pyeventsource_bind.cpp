...
NB_MODULE(_pyeventsource, m) {
  nb::class_<EventSource>(m, "EventSource")
      ...
      .def("__iter__", [](EventSource &source) {
        return nb::make_iterator<nb::rv_policy::move>(
            nb::type<EventSource>(), "EventSourceIterator", source.begin(),
            source.end());
      });
  nb::class_<SimulationConfiguration>(m, "SimulationConfiguration")
      ...
      .def_ro("prod_site_B_declination",
              &SimulationConfiguration::prod_site_B_declination)
      .def_ro("prod_site_B_inclination",
              &SimulationConfiguration::prod_site_B_inclination)
      ...
      .def_ro("detector_prog_start",
              &SimulationConfiguration::detector_prog_start)
      ...
      .def_ro("max_viewcone_radius",
              &SimulationConfiguration::max_viewcone_radius)
      .def_ro("min_viewcone_radius",
              &SimulationConfiguration::min_viewcone_radius)
      ...
      .def_ro("corsika_iact_options",
              &SimulationConfiguration::corsika_iact_options)
      .def_ro("corsika_low_E_model",
              &SimulationConfiguration::corsika_low_E_model)
      .def_ro("corsika_high_E_model",
              &SimulationConfiguration::corsika_high_E_model)
      ...
      .def_ro("corsika_low_E_detail",
              &SimulationConfiguration::corsika_low_E_detail)
      .def_ro("corsika_high_E_detail",
              &SimulationConfiguration::corsika_high_E_detail)
  ...
  m.def("initialize_logger", &initialize_logger, nb::arg("log_level") = "info",
        nb::arg("log_file") = "");
  ...
  nb::class_<RootEventSource, EventSource>(m, "RootEventSource")
      .def(nb::init<const std::string &, int64_t, std::vector<int>, bool>(),
           nb::arg("filename"), nb::arg("max_events") = -1,
           nb::arg("subarray") = std::vector<int>{},
           nb::arg("load_subarray_from_env") = false)
  ...
  nb::class_<SimtelEventSource, EventSource>(m, "SimtelEventSource")
      ...
      .def("__repr__", [](SimtelEventSource &self) {
        return fmt::format("SimtelEventSource(filename={})",
                           self.input_filename);
      });
  nb::class_<LACT1EventSource, EventSource>(m, "LACT1EventSource")
      .def(nb::init<const std::string &, const std::string &, int64_t,
                    std::vector<int>, bool>(),
           nb::arg("filename"), nb::arg("camera_config_file"),
           nb::arg("max_events") = -1, nb::arg("subarray") = std::vector<int>{},
           nb::arg("load_simulated_showers") = false)
      .def("__repr__",
           [](LACT1EventSource &self) {
             return fmt::format("LACT1EventSource(filename={})",
                                self.input_filename);
           })
      .def("__getitem__", &LACT1EventSource::operator[]);
