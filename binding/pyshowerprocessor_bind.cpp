...
void bind_showerprocessor(nb::module_ &m) {
  nb::class_<ShowerProcessor>(m, "ShowerProcessor")
      ...
      .def(nb::init<const SubarrayDescription &, const std::string &>(),
           nb::arg("subarray"), nb::arg("config"))
      .def("__call__",
           [](ShowerProcessor &self, ArrayEvent &event) { self(event); });
  nb::class_<ImageQuery>(m, "ImageQuery")
      .def(nb::init<const std::string &>(), nb::arg("config"))
      .def("__call__",
           [](ImageQuery &self, const ImageParameters &image_parameters) {
  ...
  nb::class_<Reconstructor>(m, "Reconstructor")
      ...
      .def_ro("array_pointing_direction",
              &Reconstructor::array_pointing_direction)
      .def("__call__",
           [](Reconstructor &self, ArrayEvent &event) { self(event); });
  nb::class_<GeometryReconstructor, Reconstructor>(m, "GeometryReconstructor")
      .def(nb::init<const SubarrayDescription &>(), nb::arg("subarray"))
      .def(nb::init<const SubarrayDescription &, const std::string &>(),
           nb::arg("subarray"), nb::arg("config_str"))
      .def("__call__",
           [](GeometryReconstructor &self, ArrayEvent &event) { self(event); })
      ...
      .def("get_tiled_tel_position",
           &GeometryReconstructor::get_tiled_tel_position);
}
...
void bind_coordinates(nb::module_ &m) {
  ...
  nb::class_<TelescopeFrame>(m, "TelescopeFrame")
      .def(nb::init<double, double>(), nb::arg("azimuth"), nb::arg("altitude"))
      .def("transform_to", [](TelescopeFrame &self, const Point2D &point,
                              const AltAzFrame &target) {
        return self.transform_to(point, target);
      });
  nb::class_<CartesianPoint>(m, "CartesianPoint")
      .def(nb::init<double, double, double>(), nb::arg("x"), nb::arg("y"),
           nb::arg("z"))
      .def("transform_to_tilted",
           [](CartesianPoint &self, const TiltedGroundFrame &target) {
  ...
  nb::class_<TiltedGroundFrame, TelescopeFrame>(m, "TiltedGroundFrame")
      .def(nb::init<double, double>(), nb::arg("azimuth"), nb::arg("altitude"))
      .def("transform_to", [](TiltedGroundFrame &self, const Point2D &point,
                              const AltAzFrame &target) {
        return self.transform_to(point, target);
      });
  nb::class_<AltAzFrame>(m, "AltAzFrame")
      .def(nb::init<>())
      .def("transform_to",
           [](const AltAzFrame &self, const SphericalRepresentation &point,
              const TelescopeFrame &target) {
             return self.transform_to(point, target);
           });
  nb::class_<SkyDirection<AltAzFrame>>(m, "SkyDirection")
      .def(nb::init<AltAzFrame, double, double>(),
           nb::arg("frame") = AltAzFrame(), nb::arg("azimuth"),
           nb::arg("altitude"))
      .def_prop_ro(
          "azimuth",
          [](SkyDirection<AltAzFrame> &self) { return self.position.azimuth; })
      .def_prop_ro(
          "altitude",
          [](SkyDirection<AltAzFrame> &self) { return self.position.altitude; })
      .def("transform_to",
           [](SkyDirection<AltAzFrame> &self, const TelescopeFrame &target) {
             return self.transform_to(target);
           });
  nb::class_<SkyDirection<TelescopeFrame>>(m, "TelescopeOffset")
      .def(nb::init<TelescopeFrame, double, double>(), nb::arg("frame"),
           nb::arg("offset_x"), nb::arg("offset_y"))
      .def_prop_ro(
          "x_off",
          [](SkyDirection<TelescopeFrame> &self) { return self.position.x(); })
