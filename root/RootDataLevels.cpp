...
TTree *RootEventIndex::initialize_write(const std::string &name,
                                        const std::string &title) {
  TTree *tree = new TTree(name.c_str(), title.c_str());
  tree->Branch("event_id", &event_id);
  tree->Branch("telescopes", &telescopes);
  tree->Branch("mjd_int", &mjd_data.mjd_int);
  tree->Branch("mjd_double", &mjd_data.mjd_double);
  tree->Branch("rabbitTime", &mjd_data.RabitTime);
  tree->Branch("rabbittime", &mjd_data.Rabbittime);
  return tree;
}
...
void RootEventIndex::initialize_read(TTree *tree) {
  ...
  tree->SetBranchAddress("mjd_int", &mjd_data.mjd_int);
  tree->SetBranchAddress("mjd_double", &mjd_data.mjd_double);
  tree->SetBranchAddress("rabbitTime", &mjd_data.RabitTime);
  tree->SetBranchAddress("rabbittime", &mjd_data.Rabbittime);
}
...
ArrayEvent RootEventHelper::get_event() {
  ...
  event.mjd = root_event_index->mjd_data;
...
TelescopeDescription RootConfigHelper::get_telescope_description(int ientry) {
  ...
  return TelescopeDescription{
      .camera_description =
          CameraDescription{.camera_name =
                                root_camera_geometry_entry.camera_name,
