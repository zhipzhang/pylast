...
REGISTER_WRITER(root, [](EventSource &source, const std::string &filename) {
  return std::make_unique<RootWriter>(source, filename);
});
...
void RootWriter::write_atmosphere_model() {
  ...
  RVecD alt_km(source.atmosphere_model->alt_km.data(),
               source.atmosphere_model->alt_km.size());
  RVecD rho(source.atmosphere_model->rho.data(),
            source.atmosphere_model->rho.size());
  RVecD thick(source.atmosphere_model->thick.data(),
              source.atmosphere_model->thick.size());
  RVecD refidx_m1(source.atmosphere_model->refidx_m1.data(),
                  source.atmosphere_model->refidx_m1.size());
  ...
  std::unique_ptr<TTree> tree =
      std::make_unique<TTree>("atmosphere_model", "atmosphere model");
...
- telescope_description(camera_geometry, camera_readout(most of time is not
needed for writing) optics_description)
- tel_positions
- reference_position

Try to write subarray description to a tree and each entry is a telescope
description.
*/
void RootWriter::write_subarray() {
  ...
  RVecD reference_position(source.subarray->reference_position.data(),
                           source.subarray->reference_position.size());
  ...
  dir->cd();
  std::unique_ptr<TTree> ref_tree =
      std::make_unique<TTree>("reference_position", "Array reference position");
  ref_tree->Branch("position", &reference_position);
  ref_tree->Fill();
  ref_tree->Write();

  // Create tree for telescope positions
  std::unique_ptr<TTree> tel_pos_tree =
      std::make_unique<TTree>("tel_positions", "Telescope positions");
  int tel_id = 0;
  RVecD tel_position;
  ...
  for (const auto &id : ordered_tel_ids) {
    tel_id = id;
    tel_position = RVecD(source.subarray->tel_positions[id].data(),
                         source.subarray->tel_positions[id].size());
    tel_pos_tree->Fill();
  }
  tel_pos_tree->Write();

  // Considering the optics description is quite simple, we can just write it to
  // a tree
  std::unique_ptr<TTree> optics_tree =
      std::make_unique<TTree>("optics", "Telescope optics information");
  ...
  std::unique_ptr<TTree> camera_geometry_tree =
      std::make_unique<TTree>("geometry", "Camera geometry information");
  config_helper.root_camera_geometry = RootCameraGeometry();
  config_helper.root_camera_geometry->initialize_write(
      camera_geometry_tree.get());
  auto &root_camera_geometry = config_helper.root_camera_geometry.value();
  // Write camera geometry for each telescope
  std::unique_ptr<TTree> camera_readout_tree = std::make_unique<TTree>(
      "readout", "Telescope camera readout information");
  config_helper.root_camera_readout = RootCameraReadout();
  config_helper.root_camera_readout->initialize_write(
      camera_readout_tree.get());
  auto &root_camera_readout = config_helper.root_camera_readout.value();
  for (const auto &id : ordered_tel_ids) {
    if (!source.subarray->tels.count(id))
      continue;

    const auto &camera_geom =
        source.subarray->tels.at(id).camera_description.camera_geometry;
    const auto &camera_readout =
        source.subarray->tels.at(id).camera_description.camera_readout;
...
void RootWriter::write_all_simulation_shower(
    const SimulatedShowerArray &shower_array) {
  if (!file) {
    throw std::runtime_error("file not open");
  }
...
void RootWriter::write_simulation_config() {
  ...
  if (!source.simulation_config.has_value()) {
    spdlog::warn("simulation configuration not set, skipping writing "
  ...
  dir->cd();
  std::unique_ptr<TTree> tree =
      std::make_unique<TTree>("simulation_config", "Simulation configuration");
  SimulationConfiguration config;
  initialize_simulation_config_branches(*tree, config);
  config = source.simulation_config.value();
  spdlog::warn("Writing simulation configuration: run_number = {}, "
...
void RootWriter::write_simulated_camera(const ArrayEvent &event,
                                        bool write_image) {
  if (!file) {
    throw std::runtime_error("file not open");
  }
  ...
  if (!sim_tree) {
    ...
    if (write_image) {
      ...
      sim_tree->Branch("fake_image_mask",
                       &helper.root_simulation_camera->fake_image_mask);
    }
  }
...
void RootWriter::write_event_index(const ArrayEvent &event) {
  ...
  if (!index_tree) {
    ...
    auto tree = helper.root_event_index->initialize_write(
        "event_index", "Event index for all data levels");
  ...
  helper.root_event_index->mjd_data = event.mjd;
  ...
  helper.root_event_index->telescopes =
...
void RootWriter::write_pointing(const ArrayEvent &event) {
  ...
  if (!pointing_tree) {
    ...
    dir->cd();
    auto tree =
        new TTree("pointing", "Array and telescope pointing information");
    pointing_tree = tree;
...
void RootWriter::initialize_simulation_config_branches(
    TTree &tree, SimulationConfiguration &config) {
...
template <typename T>
void RootWriter::initialize_data_level(const std::string &level_name,
                                       std::optional<T> &data_level) {
  TDirectory *dir = get_or_create_directory("events/" + level_name);
  if (!dir) {
    throw std::runtime_error("failed to create directory: events/" +
                             level_name);
  }
  dir->cd();
  data_level = T();
...
void RootWriter::write_statistics(const Statistics &statistics, bool last) {
  ...
  for (const auto &[name, hist] : hist.histograms) {
    if (hist->get_dimension() == 1) {
      ...
      auto new_hist =
          new TH1F(("h" + std::to_string(ihist)).c_str(), name.c_str(),
                   h1d->bins(), h1d->get_low_edge(), h1d->get_high_edge());
      ...
      auto new_hist = new TH2F(
          ("h" + std::to_string(ihist)).c_str(), name.c_str(), h2d->x_bins(),
          h2d->get_x_low_edge(), h2d->get_x_high_edge(), h2d->y_bins(),
          h2d->get_y_low_edge(), h2d->get_y_high_edge());
    ...
    } else if (hist->get_dimension() == 0) // Profile1D
    {
      ...
      auto new_hist =
          new TProfile(("h" + std::to_string(ihist)).c_str(), name.c_str(),
                       h1d->bins(), h1d->get_low_edge(), h1d->get_high_edge());
      for (int i = 0; i < h1d->bins(); i++) {
