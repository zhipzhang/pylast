...
json default_config() {
  ...
  config["image_processor"]["TailcutsCleaner"]["min_number_picture_neighbors"] =
      2;

  // Shower reconstruction defaults
  config["shower_processor"]["GeometryReconstructionTypes"] =
      std::vector<std::string>{"HillasReconstructor"};

  // Image Cuts
  config["shower_processor"]["MLParticleClassifier"] = json::object();
  config["shower_processor"]["MLParticleClassifier"]["ImageQuery"] =
  ...
  config["shower_processor"]["HillasReconstructor"]["ImageQuery"] =
      "hillas_intensity > 100 && leakage_intensity_width_2 < 0.3 && "
      "hillas_width > 0 && morphology_n_pixels >= 5";
  config["shower_processor"]["HillasReconstructor"]["use_fake_hillas"] = true;
  config["shower_processor"]["HillasWeightedReconstructor"]["ImageQuery"] =
      "hillas_intensity > 100 && leakage_intensity_width_2 < 0.3 && "
      "hillas_width > 0 && morphology_n_pixels >= 5";
  config["shower_processor"]["MLEnergyReconstructor"] = json::object();
  config["shower_processor"]["MLEnergyReconstructor"]["ImageQuery"] =
      "hillas_intensity > 100 && leakage_intensity_width_2 < 0.3 && "
      "hillas_width > 0 && morphology_n_pixels >= 5";
...
int main(int argc, const char *argv[]) {
  args::ArgumentParser parser("Process multiple input files and save results "
  ...
  args::ValueFlagList<std::string> input_files(
      parser, "input", "Input file path (can be specified multiple times)",
      {'i', "input"});
  args::ValueFlagList<std::string> output_files(
      parser, "output", "Output file path (can be specified multiple times)",
      {'o', "output"});
  args::ValueFlag<std::string> config_file(
      parser, "config",
      "Config file path (If not provided, default config will be used)",
      {'c', "config"});
  args::ValueFlag<std::string> max_leakage2(
      parser, "max_leakage2", "Max leakage2 for hillas reconstruction",
      {'l', "max-leakage2"});
  args::ValueFlag<std::string> subarray(
      parser, "subarray",
      "Specify telescopes to use (comma-separated list, e.g., '1,2,3,4')",
      {'s', "subarray"});
  ...
  if (!config_file_path.empty()) {
    ...
    if (!f.is_open()) {
      std::cerr << "Error: Could not open config file: " << config_file_path
                << std::endl;
      return 1;
    }
    f >> config;
  ...
  if (max_leakage2) {
    std::string leakage_query = "leakage_intensity_width_2 < " +
                                max_leakage2.Get() +
  ...
  if (subarray) {
    ...
    while ((pos = subarray_str.find(',')) != std::string::npos) {
      ...
      try {
        ...
        std::cerr << "Error: Invalid telescope ID in subarray list: " << token
                  << std::endl;
        return 1;
      }
    ...
    try {
      ...
      std::cerr << "Error: Invalid telescope ID in subarray list: "
                << subarray_str << std::endl;
      return 1;
    }
  }
  ...
  for (size_t i = 0; i < inputs.size(); i++) {
    ...
    try {
      ...
      if (!tel_ids.empty()) {
        source =
            std::make_unique<SimtelEventSource>(input_file, -1, tel_ids, false);
      } else {
        source = std::make_unique<SimtelEventSource>(input_file, -1,
      ...
      auto direction_error_versus_energ_hist =
          make_regular_histogram_2d<float>(-1, 3, 60, 0, 1, 1000);

      // Initialize processors
      auto calibrator =
          std::make_unique<Calibrator>(*source->subarray, config["calibrator"]);
      auto image_processor = std::make_unique<ImageProcessor>(
          *source->subarray, config["image_processor"]);
      auto shower_processor = std::make_unique<ShowerProcessor>(
          *source->subarray, config["shower_processor"]);
      auto data_writer = std::make_unique<DataWriter>(*source, output_file,
                                                      config["data_writer"]);
      ...
      statistics.add_histogram("Direction Error(deg) versus True Energy(TeV)",
                               direction_error_versus_energ_hist);
      ...
      statistics.add_histogram("log10(True Energy(TeV))",
