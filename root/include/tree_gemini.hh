...
private:
  ...
  static void branch_impl(TTree *tree, T &data_struct,
                          std::index_sequence<Is...>,
                          std::string prefix_name = "") {
    (
        [&]() {
          using FieldType =
              std::remove_reference_t<decltype(pfr::get<Is>(data_struct))>;
  ...
  template <typename T, std::size_t... Is>
  static void set_branch_addresses_impl(TTree *tree, T &data_struct,
                                        std::index_sequence<Is...>,
                                        std::string prefix_name = "") {
    (
        [&]() {
          ...
          if constexpr (std::is_fundamental_v<FieldType>) {
            tree->SetBranchAddress(branch_name_str.c_str(),
                                   &pfr::get<Is>(data_struct));
          }
        }(),
        ...);
  }
...
public:
   ...
   */
  template <typename T>
  static void branch(TTree *tree, T &data_struct,
                     std::string prefix_name = "") {
    branch_impl(tree, data_struct,
                std::make_index_sequence<pfr::tuple_size_v<T>>{}, prefix_name);
  }
   ...
   */
  template <typename T>
  static void set_branch_addresses(TTree *tree, T &data_struct,
                                   std::string prefix_name = "") {
    set_branch_addresses_impl(tree, data_struct,
...
#ifndef MYTTREE_READER_H
#define MYTTREE_READER_H
...
// --- 用于创建“混合存储”元组的元编程助手 ---
template <typename T,
          typename Indices = std::make_index_sequence<pfr::tuple_size_v<T>>>
...
private:
  ...
  using field_type =
      std::remove_reference_t<decltype(pfr::get<I>(std::declval<T &>()))>;

public:
  using type = std::tuple<
      std::conditional_t<std::is_fundamental_v<field_type<Is>>, field_type<Is>,
...
template <typename T> class MyTTreeReader {
private:
  TTree *m_tree;
  ...
  template <std::size_t... Is> void setup_branches(std::index_sequence<Is...>) {
    // 使用 C++17 的折叠表达式和逗号运算符来遍历所有成员
    (void)std::initializer_list<int>{(
        [this] {
          // 获取当前字段的真实类型
          using FieldType = std::remove_reference_t<decltype(pfr::get<Is>(
              std::declval<T &>()))>;
          const char *branch_name = pfr::get_name<Is, T>().data();
          ...
          if constexpr (std::is_fundamental_v<FieldType>) {
            ...
            m_tree->SetBranchAddress(branch_name,
                                     &std::get<Is>(m_hybrid_storage));
          } else {
...
public:
  explicit MyTTreeReader(TTree *tree) : m_tree(tree) {
    if (!m_tree) {
