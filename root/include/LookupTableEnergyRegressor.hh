/**
 * @file LookupTableEnergyRegressor.hh
 * @author Zach Peng (zhipzhang@mail.ustc.edu.cn)
 * @brief Classical Method based on lookuptable to reconstruct the energy
 ...
 */
class LookupTableEnergyRegressor : public Reconstructor {
public:
  LookupTableEnergyRegressor(const json &config) : Reconstructor(config) {
  ...
  LookupTableEnergyRegressor(const std::string &config_str)
  ...
  ~LookupTableEnergyRegressor() = default;
  void operator()(ArrayEvent &event) override;
  void load_lookup_table(const std::string &lookup_table_path);
  std::string name() const override { return "LookupTableEnergyRegressor"; }
  ReconstructedEnergy energy_reco;

private:
  std::unique_ptr<TFile> lookup_table_file;
  TH2D *mean_lookup_table;
  TH2D *sigma_lookup_table;

  /* Whether to use sigma as weight, if not, intensity will be used as weight */
  bool use_sigma_as_weight = false;
};