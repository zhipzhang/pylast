...
template <typename TelConfiguration> class RootTelConfiguration {
public:
  int tel_id;
...
public:
  ...
  RootCameraReadout &operator=(CameraReadout &&other) noexcept {
    ...
    reference_pulse_shape = RVecD(config.reference_pulse_shape.data(),
                                  config.reference_pulse_shape.size());
    return *this;
  }
  ...
  void initialize_read_pointer(TTree *tree) override {
    ...
    if (tree->GetBranch("reference_pulse_shape") != nullptr) {
      tree->SetBranchAddress("reference_pulse_shape",
                             &reference_pulse_shape_ptr);
    }
  }
  CameraReadout &&get_entry(int ientry) {
    ...
    config.reference_pulse_shape = Eigen::Map<Eigen::VectorXd>(
        reference_pulse_shape_ptr->data(), reference_pulse_shape_ptr->size());
    return std::move(config);
  }
...
public:
  ...
  CameraGeometry get_entry(int ientry) {
    read_tree->GetEntry(ientry);
    Eigen::VectorXd pix_x =
        Eigen::Map<Eigen::VectorXd>(pix_x_ptr->data(), pix_x_ptr->size());
    Eigen::VectorXd pix_y =
        Eigen::Map<Eigen::VectorXd>(pix_y_ptr->data(), pix_y_ptr->size());
    Eigen::VectorXd pix_area =
        Eigen::Map<Eigen::VectorXd>(pix_area_ptr->data(), pix_area_ptr->size());
    Eigen::VectorXi pix_type =
        Eigen::Map<Eigen::VectorXi>(pix_type_ptr->data(), pix_type_ptr->size());
    return CameraGeometry(*camera_name_ptr, config.num_pixels, pix_x, pix_y,
                          pix_area, pix_type, config.cam_rotation);
  }
...
template <typename DataLevels> class NewRootDataLevels {
public:
...
public:
  ...
  RootSimulatedCamera &operator=(SimulatedCamera &&other) noexcept {
    ...
    true_image = std::move(
        RVecI(datalevels.true_image.data(), datalevels.true_image.size()));
    fake_image = std::move(
        RVecD(datalevels.fake_image.data(), datalevels.fake_image.size()));
    fake_image_mask = std::move(RVecB(datalevels.fake_image_mask.data(),
                                      datalevels.fake_image_mask.size()));
  ...
  void initialize_internal_structure(TTree *tree) override {
    ...
    TTreeSerializer::branch(tree, fake_image_parameters.concentration,
                            "concentration");
    TTreeSerializer::branch(tree, fake_image_parameters.morphology,
                            "morphology");
  ...
  void initialize_read_pointer(TTree *tree) override {
    ...
    TTreeSerializer::set_branch_addresses(tree, fake_image_parameters.hillas,
                                          "hillas");
    TTreeSerializer::set_branch_addresses(tree, fake_image_parameters.leakage,
                                          "leakage");
    TTreeSerializer::set_branch_addresses(
        tree, fake_image_parameters.concentration, "concentration");
    TTreeSerializer::set_branch_addresses(
        tree, fake_image_parameters.morphology, "morphology");
    TTreeSerializer::set_branch_addresses(tree, fake_image_parameters.intensity,
                                          "intensity");
    TTreeSerializer::set_branch_addresses(tree, fake_image_parameters.extra,
                                          "extra");
  }
  void update_after_get_entry() override {
    ...
    if (fake_image_ptr) {
      datalevels.fake_image = Eigen::Map<Eigen::VectorXd>(
          fake_image_ptr->data(), fake_image_ptr->size());
    }
    if (fake_image_mask_ptr) {
      datalevels.fake_image_mask = Eigen::Map<Eigen::Vector<bool, -1>>(
          fake_image_mask_ptr->data(), fake_image_mask_ptr->size());
    }
...
public:
  ...
  RootR0Camera &operator=(R0Camera &&other) noexcept {
    datalevels = std::move(other);
    low_gain_waveform =
        std::move(RVec<uint16_t>(datalevels.waveform[0].data(),
                                 datalevels.n_pixels * datalevels.n_samples));
    high_gain_waveform =
        std::move(RVec<uint16_t>(datalevels.waveform[1].data(),
                                 datalevels.n_pixels * datalevels.n_samples));
    low_gain_waveform_sum = std::move(
        RVec<uint32_t>(datalevels.waveform_sum[0].data(), datalevels.n_pixels));
    high_gain_waveform_sum = std::move(
        RVec<uint32_t>(datalevels.waveform_sum[1].data(), datalevels.n_pixels));
    return *this;
  }
  void initialize_internal_structure(TTree *tree) override {
  ...
  void initialize_read_pointer(TTree *tree) override {
    ...
    if (tree->GetBranch("low_gain_waveform_sum") != nullptr) {
      tree->SetBranchAddress("low_gain_waveform_sum",
                             &low_gain_waveform_sum_ptr);
    }
    if (tree->GetBranch("high_gain_waveform_sum") != nullptr) {
      tree->SetBranchAddress("high_gain_waveform_sum",
                             &high_gain_waveform_sum_ptr);
    }
  }
  void update_after_get_entry() override {
    if (low_gain_waveform_ptr) {
      datalevels.waveform[0] =
          Eigen::Map<Eigen::Matrix<uint16_t, -1, -1, Eigen::RowMajor>>(
              low_gain_waveform_ptr->data(), datalevels.n_pixels,
              datalevels.n_samples);
    }
    if (high_gain_waveform_ptr) {
      datalevels.waveform[1] =
          Eigen::Map<Eigen::Matrix<uint16_t, -1, -1, Eigen::RowMajor>>(
              high_gain_waveform_ptr->data(), datalevels.n_pixels,
              datalevels.n_samples);
    }
    if (low_gain_waveform_sum_ptr) {
      datalevels.waveform_sum[0] = Eigen::Map<Eigen::Vector<uint32_t, -1>>(
          low_gain_waveform_sum_ptr->data(), low_gain_waveform_sum_ptr->size());
    }
    if (high_gain_waveform_sum_ptr) {
      datalevels.waveform_sum[1] = Eigen::Map<Eigen::Vector<uint32_t, -1>>(
          high_gain_waveform_sum_ptr->data(),
          high_gain_waveform_sum_ptr->size());
    }
  }
...
public:
  ...
  RootR1Camera &operator=(R1Camera &&other) noexcept {
    datalevels = std::move(other);
    waveform = std::move(RVecD(datalevels.waveform.data(),
                               datalevels.n_pixels * datalevels.n_samples));
    gain_selection =
        std::move(RVecI(datalevels.gain_selection.data(), datalevels.n_pixels));
    return *this;
  }
  void initialize_internal_structure(TTree *tree) override {
    ...
    if (waveform_ptr) {
      datalevels.waveform =
          Eigen::Map<Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(
              waveform_ptr->data(), datalevels.n_pixels, datalevels.n_samples);
    }
    if (gain_selection_ptr) {
      datalevels.gain_selection = Eigen::Map<Eigen::VectorXi>(
          gain_selection_ptr->data(), gain_selection_ptr->size());
    }
  }
...
public:
  ...
  RootDL0Camera &operator=(DL0Camera &&other) noexcept {
    ...
    peak_time = std::move(
        RVecD(datalevels.peak_time.data(), datalevels.peak_time.size()));
    return *this;
  }
  void initialize_internal_structure(TTree *tree) override {
  ...
  void update_after_get_entry() override {
    if (image_ptr) {
      datalevels.image =
          Eigen::Map<Eigen::VectorXd>(image_ptr->data(), image_ptr->size());
    }
    if (peak_time_ptr) {
      datalevels.peak_time = Eigen::Map<Eigen::VectorXd>(peak_time_ptr->data(),
                                                         peak_time_ptr->size());
    }
  }
...
public:
  ...
  RootDL1Camera &operator=(DL1Camera &&other) noexcept {
    ...
    peak_time = std::move(
        RVecF(datalevels.peak_time.data(), datalevels.peak_time.size()));
  ...
  void initialize_internal_structure(TTree *tree) override {
    ...
    TTreeSerializer::branch(tree, datalevels.image_parameters.leakage,
                            "leakage");
    TTreeSerializer::branch(tree, datalevels.image_parameters.concentration,
                            "concentration");
    TTreeSerializer::branch(tree, datalevels.image_parameters.morphology,
                            "morphology");
    TTreeSerializer::branch(tree, datalevels.image_parameters.intensity,
                            "intensity");
  ...
  void initialize_read_pointer(TTree *tree) override {
    ...
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.hillas, "hillas");
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.leakage, "leakage");
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.concentration, "concentration");
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.morphology, "morphology");
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.intensity, "intensity");
    TTreeSerializer::set_branch_addresses(
        tree, datalevels.image_parameters.extra, "extra");
  }
  void update_after_get_entry() override {
    if (image_ptr) {
      datalevels.image =
          Eigen::Map<Eigen::VectorXf>(image_ptr->data(), image_ptr->size());
    }
    if (peak_time_ptr) {
      datalevels.peak_time = Eigen::Map<Eigen::VectorXf>(peak_time_ptr->data(),
                                                         peak_time_ptr->size());
    }
    if (mask_ptr) {
      datalevels.mask = Eigen::Map<Eigen::Vector<bool, -1>>(mask_ptr->data(),
                                                            mask_ptr->size());
    }
  }
  ...
  void update_after_get_entry() override {
    ...
    for (size_t i = 0; i < reconstructor_name.size(); ++i) {
      datalevels.impact_parameters[reconstructor_name[i]] = TelImpactParameter{
          .distance = distance[i], .distance_error = distance_error[i]};
    }
  }
...
template <typename DL2RecParameter> class RootDL2RecParameter {
public:
...
public:
  ...
  RootTelMonitor &operator=(TelMonitor &&other) noexcept {
    datalevels = std::move(other);
    dc_to_pe = std::move(RVecD(datalevels.dc_to_pe.data(),
                               datalevels.n_channels * datalevels.n_pixels));
    pedestals = std::move(RVecD(datalevels.pedestal_per_sample.data(),
                                datalevels.n_channels * datalevels.n_pixels));
    return *this;
  }
  void initialize_internal_structure(TTree *tree) override {
  ...
  void update_after_get_entry() override {
    if (dc_to_pe_ptr) {
      datalevels.dc_to_pe = Eigen::Map<Eigen::MatrixXd>(
          dc_to_pe_ptr->data(), datalevels.n_channels, datalevels.n_pixels);
    }
    if (pedestals_ptr) {
      datalevels.pedestal_per_sample = Eigen::Map<Eigen::MatrixXd>(
          pedestals_ptr->data(), datalevels.n_channels, datalevels.n_pixels);
    }
  }
...
public:
  ...
  Pointing get_entry(int ientry) {
    ...
    for (size_t i = 0; i < (*tel_id_ptr).size(); ++i) {
      pointing.add_tel((*tel_id_ptr)[i],
                       PointingTelescope{.azimuth = (*tel_azimuth_ptr)[i],
...
public:
  ...
  std::unordered_map<std::string, std::optional<RootDL2RecGeometry>>
      root_dl2_rec_geometry_map;
  std::unordered_map<std::string, std::optional<RootDL2RecEnergy>>
      root_dl2_rec_energy_map;
  std::unordered_map<std::string, std::optional<RootDL2RecParticle>>
      root_dl2_rec_particle_map;
  ...
  void process_tel_data_level(std::optional<RootDataType> &root_data,
    ...
    if (root_data.has_value()) {
      int entry_number =
          root_data->compute_entry_number(root_event_index->event_id, tel_id);
      if (entry_number >= 0) {
  ...
  void process_dl2_rec_data_map(
      std::unordered_map<std::string, std::optional<RootDL2Type>> &root_map,
